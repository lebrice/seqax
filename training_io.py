...
def load_checkpoint_if_it_exists(
    ...
    return load_zarr(
        os.path.join(checkpoint_dir, step_to_str(selected_checkpoint)), state, config
    ), selected_checkpoint
...
def save_hlo_svg(filespec: str, compiled: jax.stages.Compiled):
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        subprocess.check_call(shlex.split(f"dot -Tsvg {f.name} -o{hlo_orig_svg}"))
...
def get_flops_per_device():
    ...
    if device.startswith("NVIDIA A100"):
        result = 312e12
    elif device.startswith("NVIDIA H100"):
        # source: https://www.databricks.com/blog/coreweave-nvidia-h100-part-1
        result = 989e12
    else:
        print(
